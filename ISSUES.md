...
| Docker ROS workspace build context is oversized | Improved; verified on aircraft rebuild | The aircraft rebuild transferred about 5.0 GB and reached the ROS compile stage in about 26 seconds, versus the prior ~7.6 GB ground transfer. The source-level `.dockerignore` is active; the remaining context is still larger than ideal and can be reduced further. |
