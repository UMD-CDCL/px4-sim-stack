...
| FIXED, timeout-expiration case pending | RTSP preflight deadline accounting. | The entrypoint now uses one wall-clock deadline and caps each probe and sleep to the remaining budget, so the configured 300-second policy is an elapsed-time limit. A cache-preserving rebuild and live startup passed; a deliberately delayed publisher run is still required to certify deadline expiration. | [`modules/onboard/entrypoint.sh`](../modules/onboard/entrypoint.sh) |
...
4. **Live-test the RTSP deadline fix.** The entrypoint now uses one wall-clock deadline around probe and sleep, preserving the desired 300-second policy as elapsed time. Decide separately whether it is correct to launch ROS after the stream deadline expires.
