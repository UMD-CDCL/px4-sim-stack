...
| FIXED, pending live image | RTSP preflight deadline accounting. | The entrypoint now uses one wall-clock deadline and caps each probe and sleep to the remaining budget, so the configured 300-second policy is an elapsed-time limit. A rebuilt image and live delayed-publisher run are still required to certify runtime behavior. | [`modules/onboard/entrypoint.sh`](../modules/onboard/entrypoint.sh) |
