...
| `TESTED (live 2026-09-16)` | UAS topic/node inspection | `px4sim state`/UI reports link health, MAVLink message rate, system IDs, position, altitude, heading, gimbal attitude, GPS fix, satellites, explicit `mission_phase`, and a cached per-UAS detection summary (`state`, `boxes`, `error`). Live state returned `uas11: live, boxes=0`; dedicated `px4sim uas N detections` remains the detailed box/localization front door. | `scripts/state.py`, `scripts/mavlink.py`, `scripts/tui.py`, `px4sim uas` |
