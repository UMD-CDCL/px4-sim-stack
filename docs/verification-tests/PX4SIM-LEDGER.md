...
| `PARTIAL (live 2026-09-16)` | Foxglove scoring plot telemetry | `verify/stages/90-foxglove.sh`, canonical `chimera_sim.json` | Layout and all five scorer topics are present. Live bridge verification observed `recall` and `detection_recall`; `precision`, `detection_precision`, and `position_error` carried no data in this run. Investigate conditional scorer publication/subscriber discovery before marking complete. | Foxglove stage, 32 passed / 6 failed |
