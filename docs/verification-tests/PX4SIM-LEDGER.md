...
| `PARTIAL` | Recording front door | `px4sim record start|stop|status` now provides a canonical ground-domain lifecycle and host-persisted output path; the current image lacks the MCAP rosbag storage plugin and synchronized video lifecycle, so the command fails its preflight with an explicit remediation |
