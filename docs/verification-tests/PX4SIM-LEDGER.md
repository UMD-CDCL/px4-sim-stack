...
| `TESTED (live 2026-09-16, root b2bba41)` | Raw ROI front door | `px4sim uas N raw-roi`, canonical `chimera_sim.json`, `gimbal.py` | Full flight-stage assertion accepted the coordinate and confirmed the exact target remained in `mode=roi` after movement. The packet had one unrelated early gimbal-readiness miss but raw ROI passed. | `verify/component/uas.py`, `verify/stages/55-flight.sh` |
