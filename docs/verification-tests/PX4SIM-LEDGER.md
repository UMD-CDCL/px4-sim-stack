...
| `MISSING` | Scene/origin/scenario selection easily in px4sim ui, should be able to chose from existing list, have scenarios as children to scenes (ie uroc scene can't choose lorton_casualties unless allowed), so the scenario determines both the scene, scenario, and origin | `.env`, `scripts/origin-env.sh`, scenegen | Selected scene and origin reach sim/runtime |
...
| `ORPHANED` | UAS topic/node inspection: the foxglove front door should control the drone, but this should be updated to make the drone status mroe relevant to testing. in conjunction with better node status reporting, reports that start when you start px4sim ui, and relevant reports from the drone status like detections, stream health, mavlink, bandwidth, mission phase (takeoff, transit, detect, survey, point, assess, etc, look through and find what is available to report and do it from the source) | `px4sim uas`, `scripts/state.py` | Status, heading, detections, and state are readable |
...
| Status | UI surface/action | Interaction and behavior to verify | Code path / likely issue |
|---|---|---|---|
| `CURRENT` | Stack pane | Shows services, vehicles, streams, and command output | `tui.py` panes and `state --watch` |
| `CURRENT` | Live state feed | Refreshes about every 2 seconds and retries after feed failure | `Feed`, `REPORT_PERIOD_S`, `FEED_RETRY_S` |
...
| `CURRENT` | Service selection | Tab moves to services; selected service exposes service actions | `PANES`, service rows |
| `CURRENT` | Vehicle selection | Tab moves to vehicles; selected vehicle substitutes UAS values | vehicle row/action formatting |
| `CURRENT` | Stream selection | Tab moves to streams; selected stream can be played or snapped | stream pane/actions |
...
| `CURRENT` | `N` switch scene | Prompts for scene, writes selection, refreshes | `scene`, simulator/ground |
| `CURRENT` | `T` switch targets | Prompts from available scenarios and reloads | `scenario`, simulator/ground |
| `CURRENT` | `F` move fiducial | Prompts east/north metres and changes marker | `fiducial`, simulator only |
| `CURRENT` | `V` verification | Runs the aggregate verification front door | `verify` |
| `CURRENT` | `C` code/front-door check | Runs compose/layout/doc consistency checks | `check` |
| `CURRENT` | `D` host check | Runs host/GPU/Docker/X11/preflight checks | `doctor` |
| `CURRENT` | `L` layout location | Prints selected Foxglove layout path and loading instructions | `layout` |
...
| `UNKNOWN` | Terminal width fallback | UI claims 100-column usability; narrow terminals need verification | module docstring, rendering widths |
| `UNKNOWN` | Terminal resize handling | Determine whether panes redraw safely after SIGWINCH | curses setup/render loop |
| `UNKNOWN` | Unicode/ANSI stripping | State/log output should not corrupt rows or widths | `ESCAPE_CODES`, `strip_codes` |
| `UNKNOWN` | Broken `state --watch` recovery | Feed retries while command actions continue safely | `Feed.run`, subprocess lifecycle |
| `UNKNOWN` | Long-running command output | Output cap, scrollback, and cancellation preserve useful evidence | `OUTPUT_LINES`, `Runner` |
| `UNKNOWN` | Foreground command cleanup | Viewer/console processes release sockets on Escape/q | `stop_process`, process groups |
| `UNKNOWN` | Empty fleet/stack rendering | UI remains usable when no services or UAS exist | state parsing/rendering |
| `UNKNOWN` | Real-aircraft world safety | Simulator-only keys are absent and typed commands are guarded | world filtering and `check_mode` |
...
| `ORPHANED` | UI `build`/`prepare` action | Front door supports build/prepare, but `ACTIONS` has no direct UI action | decide whether omission is intentional |
| `ORPHANED` | UI `setup`/`bootstrap` action | Front door supports setup, but UI has no setup action | decide whether omission is intentional |
| `ORPHANED` | UI `fleet` edit prompt beyond add/remove | CLI supports more fleet controls than action table | decide whether menu should expose them |
...
| `MISSING` | px4sim ui text wrapping | at least the terminal view should have text wrapping even if it means it displays less lines, ideally togglable with a pager too |
