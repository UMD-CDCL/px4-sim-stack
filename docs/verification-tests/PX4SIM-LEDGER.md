...
| `TESTED (live 2026-09-16, root checkpoint pending)` | Foxglove scoring plot telemetry | `verify/stages/90-foxglove.sh`, canonical `chimera_sim.json` | Full staged sim verified all five scorer metrics (`recall`, `precision`, `detection_recall`, `detection_precision`, `position_error`) carried data through Foxglove after 5G Drone commit `0bb58e8`; packet passed 44/0. | Foxglove stage |
...
| `TESTED (live 2026-09-16, root 8bd53ee)` | Portability manifest front door | `px4sim manifest` | Emits UTC checkpoint, world/config/layout/fleet, configured and canonical model-manifest fingerprints, repository branches and commits with dirty counts, and local Compose image IDs or digests; live output confirmed all compatibility branches and canonical `chimera_sim.json`. |
...
| `PARTIAL` | Recording front door | `px4sim record start|stop|status` provides a canonical ground-domain lifecycle and host-persisted output path; pinned rosbag2 MCAP storage is built into the prepared dependency image and live MCAP start/status/stop plus `ros2 bag info` passed. Synchronized video lifecycle remains planned |
