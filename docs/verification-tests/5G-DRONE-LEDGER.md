...
| `CURRENT` / `Tested: YES (2026-09-16, 5G Drone db780ed; rebuilt root checkpoint)` | Safety interlocks for gimbal | Pitch and yaw commands are clipped to configured Chimera limits and report requested yaw beyond the limit. ROI ingress now rejects non-finite or out-of-range latitude/longitude before latching or commanding the gimbal; valid ROI remains earth-locked and is re-aimed while moving. |
...
| `CURRENT` / `Tested: YES (2026-09-16, root 118c4e0; sim 69d5db8)` | Scoring frames | Verdict computation uses the fixed fiducial frame, and each verdict carries the exact configured gate diameter; MAVInsight consumes that wire diameter for the 3D bubble. The source contract test and full live sim checkpoint verify frame/bubble consistency. |
