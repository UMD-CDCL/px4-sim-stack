...
| `PARTIAL (command live 2026-09-16)` | `uas11_raw_roi` | Publish raw ROI | Canonical layout publishes `sensor_msgs/msg/NavSatFix` to `/uas11/raw_roi_point_cmd`; verifier front door now publishes the same contract, and live state returned `mode=roi` with the requested coordinate. Full flight-stage assertion remains pending. | Bench/Sim |
