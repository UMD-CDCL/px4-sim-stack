...
# companion reads its own entry rather than carrying a second copy that can
# disagree. A disagreement would be silent: the simulator would serve pilot<N>
# while ds_node opened rgb<N>, and the detector would simply never receive a
# frame. Set MODEL to override one vehicle. On the aircraft the launch takes
# the model from the vehicle's own number, so MODEL stays empty unless set.
MODEL=${MODEL:-}
if [ "${SIM}" = true ]; then
...
if [ "${SIM}" = true ]; then
	...
	# ---------------------------------------------------------------------
	# PX4 SITL currently has no MAVROS gimbal-device publisher. Keep the
	# simulator TF contract alive with a timestamped neutral attitude; the
	# aircraft path continues to consume real telemetry.
	...
	# ---------------------------------------------------------------------
	# The aircraft. rcam serves every camera as a socket, all of them at
	# once, so any DeepStream socket says the cameras are up. The launch
	...
	until ls /tmp/*ds_nv.sock >/dev/null 2>&1; do
		if [ "${waited}" -ge "${STREAM_WAIT_S}" ]; then
			echo "uas${UAS_NUM}: no /tmp/*ds_nv.sock after ${STREAM_WAIT_S}s. Is rcam.service running?" >&2
			echo "The detector will start anyway and fail to open its camera." >&2
			break
		fi
		[ "${waited}" = 0 ] && echo "waiting for the rcam sockets in /tmp"
		sleep 5
		waited=$((waited + 5))
	done
	if ls /tmp/*ds_nv.sock >/dev/null 2>&1; then
