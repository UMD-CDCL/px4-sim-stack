...
Why this exists
---------------
PX4 SITL opens its ground station link as a UDP server on port 18570 and waits.
It learns where to send telemetry from the first packet that arrives. It never
speaks first.

mavlink-router only forwards. It never originates a packet. So with a router in
the middle and no ground station attached, both sides wait and no telemetry ever
moves.

This program breaks the deadlock. It sends a MAVLink 2 HEARTBEAT once a second
to a private endpoint on the router, as system 255, component 190. The router
forwards that to PX4, PX4 records the router as its peer, and telemetry starts.
It then reaches every consumer, whether or not a ground station is running.
...
# A ground station identity that will not collide with the vehicle (id 1).
SYSTEM_ID = 255
...
def main() -> int:
    ...
    ap.add_argument("--port", type=int, default=14553)
