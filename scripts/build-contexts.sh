...
bench_patch="$PWD/patches/umd-uas-bench-sim-frame.patch"
if [ -f "$bench_patch" ]; then
    patch -d "$root/umd_uas" -p1 < "$bench_patch"
else
    echo "bench frame patch unavailable; staging upstream umd_uas unchanged" >&2
fi
