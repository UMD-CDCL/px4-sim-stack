...
ACTIONS = (
    ...
    Action(STACK, "", "enable GPS-free BENCH MODE (select this menu item)",
           ...
           refresh=True, worlds=(AIRCRAFT, GROUND)),
    Action(STACK, "", "disable GPS-free BENCH MODE and stop the stack (select this menu item)",
           ("bench", "disable"),
           confirm="Stop all containers and disable bench mode before flight?", refresh=True,
...
def vehicle_words(vehicle: dict) -> list[tuple[str, str]]:
    ...
    if link == "up":
        ...
        parts.append((f"↓{rx / 1000:.1f}M" if isinstance(rx, (int, float)) else "↓-", "faint"))
        tx = vehicle.get("tx_kbits")
        parts.append((f"↑{tx / 1000:.1f}M" if isinstance(tx, (int, float)) else "↑-", "faint"))
...
class Console:
    ...
    def share_rows(self, space: int) -> dict[str, int]:
        ...
        """
        # Vehicle identity and health are the primary ground/aircraft view.
        # Streams remain visible, but their rows are deliberately compact:
